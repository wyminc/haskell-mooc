...
palindromify s
    | s == reverse s = s
    | otherwise = palindromify $ chopFrontBack s
  where
    chopFrontBack :: String -> String
    chopFrontBack str = tail $ take (length str - 1) str
...
greet first last = case last of
    Nothing -> hello ++ first ++ punctuation
    Just last -> hello ++ first ++ " " ++ last ++ punctuation
  where
    hello :: String
    hello = "Hello, "
    punctuation :: String
