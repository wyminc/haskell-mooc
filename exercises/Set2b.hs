...
myGcd 0 b = b
myGcd a 0 = a
myGcd a b = myGcd result smaller
  where
    smaller :: Integer
    smaller = if a < b then a else b
    bigger :: Integer
    bigger = if smaller == a then b else a
    result :: Integer
...
countdown n = countdown' n ""
  where
    ...
    printNumber n = show n ++ "... "
    countdown' :: Integer -> String -> String
    countdown' 1 s = start ++ s ++ printNumber 1 ++ end
    countdown' n s = countdown' (n - 1) (s ++ printNumber n)
...
smallestDivisor n = smallestDivisor' n 2
  where
    ...
    smallestDivisor' numerator divisor
        | numerator `mod` divisor == 0 = divisor
        | otherwise = smallestDivisor' numerator (divisor + 1)
