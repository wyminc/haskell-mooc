...
firstHalf x = take half x
  where
    strLength :: Int
    strLength = length x
    half :: Int
    half
        | even strLength = strLength `div` 2
        | otherwise = strLength `div` 2 + 1
...
joinToLength len strs = concatMap (\x -> [x ++ s | s <- strs, length (x ++ s) == len]) strs
...
multiCompose [] n = n
multiCompose (f : fs) n = f $ multiCompose fs n
...
interpreter commands = interpreter' commands 0 0
  where
    ...
    interpreter' (command : commands) x y = case command of
        "up" -> interpreter' commands x (y + 1)
        "down" -> interpreter' commands x (y - 1)
        "left" -> interpreter' commands (x - 1) y
        "right" -> interpreter' commands (x + 1) y
        "printX" -> show x : interpreter' commands x y
        "printY" -> show y : interpreter' commands x y
        _ -> []