...
sums i = sums' 1 0 i
  where
    ...
    sums' nextValue 0 max = nextValue : sums' nextValue 2 max
    sums' nextValue incrementBy max =
        let newNextValue = nextValue + incrementBy
         in if incrementBy == max
                then [newNextValue]
                else newNextValue : sums' newNextValue (incrementBy + 1) max
...
sumsOf xs = sumsOf' xs 0
  where
    sumsOf' :: [Int] -> Int -> [Int]
    sumsOf' [] _ = []
    sumsOf' (v : vs) 0 = v : sumsOf' vs v
    sumsOf' (v : vs) amountToBeAdded =
        let newAmoutToBeAdded = v + amountToBeAdded
         in newAmoutToBeAdded : sumsOf' vs newAmoutToBeAdded
