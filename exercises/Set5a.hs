...
setName newName (MkPerson age name) = MkPerson age newName

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge newAge (MkPerson age name) = MkPerson newAge name
...
tick :: UpDown -> UpDown
tick (Up up) = Up (up + 1)
tick (Down down) = Down (down - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Up up) = Down up
toggle (Down down) = Up down
...
rgb (Invert x) = map (`subtract` 1) (rgb x)
...
prettyPrint End = ""
prettyPrint (O bins) = prettyPrint bins ++ "0"
prettyPrint (I bins) = prettyPrint bins ++ "1"

fromBin :: Bin -> Int
fromBin bin = foldl (\acc x -> acc * 2 + digitToInt x) 0 (prettyPrint bin)

toBin :: Int -> Bin
toBin 0 = O End
toBin dec = toBin' dec (I End)
  where
    toBin' :: Int -> Bin -> Bin
    toBin' 1 bin = bin
    toBin' int bin = toBin' (int - 1) (inc bin)
