...
treeMax tree = treeMax' tree 0
 where
  ...
  treeMax' (Node a b c) acc
    | acc > a = max (treeMax' b acc) (treeMax' c acc)
    | otherwise = max (treeMax' b a) (treeMax' c a)
...
isOrdered (Node a b c)
  | allValues (<= a) b && allValues (>= a) c = isOrdered b && isOrdered c
  | otherwise = False
...
walk (StepL : xs) (Node a b _) = walk xs b
walk (StepR : xs) (Node a _ c) = walk xs c
...
set (StepL : xs) val (Node a b c) = Node a (set xs val b) c
set (StepR : xs) val (Node a b c) = Node a b (set xs val c)
...
search val tree =
  let search' :: Eq a => a -> Tree a -> [Step] -> Maybe [Step]
      search' _ Empty _ = Nothing
      search' val (Node a b c) steps
        | val == a = Just steps
        | otherwise = if isNothing leftResult then rightResult else leftResult
       where
        leftResult = search' val b (steps ++ [StepL])
        rightResult = search' val c (steps ++ [StepR])
   in search' val tree []