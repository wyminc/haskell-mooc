...
instance Eq RationalNumber where
  ...
  (==) (RationalNumber n d) (RationalNumber n' d') = n * d' == d * n'
...
simplify (RationalNumber n d) = RationalNumber newNumerator newDenominator
 where
  cd :: Integer
  cd = gcd n d
  newNumerator :: Integer
  newNumerator = n `div` cd
  newDenominator :: Integer
  newDenominator = d `div` cd
...
instance Num RationalNumber where
  ...
  (+) (RationalNumber n d) (RationalNumber n' d') = simplify $ RationalNumber ((n * leftDivisor) + (n' * rightDivisor)) cm
   where
    cm :: Integer
    cm = lcm d d'
    leftDivisor :: Integer
    leftDivisor = cm `div` d
    rightDivisor :: Integer
    rightDivisor = cm `div` d'
  ...
  abs (RationalNumber n d)
    | n < 0 = RationalNumber (0 - n) d
    | otherwise = RationalNumber n d

  signum :: RationalNumber -> RationalNumber
  signum (RationalNumber n d) = RationalNumber (getSignum n) (getSignum d)
   where
    getSignum :: Integer -> Integer
    getSignum x
      | x == 0 = 0
      | x < 0 = -1
      | otherwise = 1
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 x = x
  stepMany s x = stepMany (s - 1) (step x)
