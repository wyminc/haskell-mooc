...
member target (Set []) = False
member target (Set (x : xs)) = target == x || member target (Set xs)

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add value (Set list) = case list of
  [] -> Set [value]
  (x : xs)
    | x == value -> Set $ x : xs
    | value < x -> Set $ value : x : xs
    | otherwise -> add x $ add value (Set xs)
...
data State = Start | AddingFlourOrSugar | AddingFlour | AddingSugar | Mixing | Baking | Error | Finished
  deriving (Eq, Show)

step :: State -> Event -> State
step Start AddEggs = AddingFlourOrSugar
step Start _ = Error
step Error _ = Error
step Finished _ = Finished
step AddingFlourOrSugar AddFlour = AddingSugar
step AddingFlourOrSugar AddSugar = AddingFlour
step AddingFlourOrSugar _ = Error
step AddingFlour AddFlour = Mixing
step AddingFlour _ = Error
step AddingSugar AddSugar = Mixing
step AddingSugar _ = Error
...
average neXs = sum neXs / (fromIntegral . length) neXs
...
passwordAllowed pw (ContainsSome str) = countIfStringContainsOtherString pw str > 0
passwordAllowed pw (DoesNotContain str) = countIfStringContainsOtherString pw str == 0
...
countIfStringContainsOtherString :: String -> String -> Int
countIfStringContainsOtherString str1 str2 = foldr (\cv accum -> if cv `elem` str2 then accum + 1 else accum) 0 str1
...
evaluate (Operation opt x y)
  | opt == "+" = evaluate x + evaluate y
  | opt == "*" = evaluate x * evaluate y
  | otherwise = 0

render :: Arithmetic -> String
render (Literal x) = show x
render (Operation opt x y) = "(" ++ render x ++ opt ++ render y ++ ")"
